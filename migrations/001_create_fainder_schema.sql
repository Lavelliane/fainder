...
-- Create document_embeddings table for AI-powered semantic search
CREATE TABLE IF NOT EXISTS document_embeddings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    document_id UUID REFERENCES documents(id) ON DELETE CASCADE,
    chunk_index INTEGER NOT NULL,
    chunk_text TEXT NOT NULL,
    chunk_embedding VECTOR(1536), -- OpenAI embedding size
...
CREATE POLICY "Users can view embeddings of their documents" ON document_embeddings
    FOR ALL USING (document_id IN (
        SELECT d.id FROM documents d 
        JOIN users u ON d.user_id = u.id 
        WHERE u.session_id = current_setting('app.current_session_id', true)
    ));
